        ...
        */
        ...
        private function copyDefaultData(Project $project)
        {
            ...
            foreach ($tablesToCopy as $table) {
                try {
                    // Get default data from main database (where project_id is null or 0)
                    $data = \DB::table($table)
                        ->where(function ($q) {
                    ...
                    if ($data->count() > 0) {
                        // SHARED DATABASE MODE: Insert with project_id in same database
                        foreach ($data as $row) {
                            $rowArray = (array) $row;
                            unset($rowArray['id']); // Let auto-increment handle ID
