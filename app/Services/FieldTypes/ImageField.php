...
class ImageField extends BaseFieldType
{
    public function render(array $config, mixed $value = null): string
    {
        ...
        $fieldHtml = "<div class=\"space-y-3\" data-image-url=\"{$escapedValue}\" x-data=\"{ imageUrl: '', fieldName: '{$name}' }\" x-init=\"imageUrl = \$el.dataset.imageUrl || ''\" @media-selected.window=\"if (\$event.detail.field === fieldName) { imageUrl = Array.isArray(\$event.detail.urls) ? \$event.detail.urls[0] : \$event.detail.urls; \$nextTick(() => { const inp = document.getElementById('{$fieldId}'); if (inp) { inp.value = imageUrl; inp.dispatchEvent(new Event('input', { bubbles: true })); } }); }\" id=\"{$fieldId}_wrapper\">";
        ...
        $fieldHtml .= "<input type=\"hidden\" name=\"{$name}\" :value=\"imageUrl\" value=\"{$escapedValue}\" id=\"{$fieldId}\">";

        // Input URL + Media Picker Button
        $fieldHtml .= '<div class="flex gap-2">';
        $fieldHtml .= "<input type=\"text\" x-model=\"imageUrl\" value=\"{$escapedValue}\" @input=\"imageUrl = \$event.target.value; \$nextTick(() => { const inp = document.getElementById('{$fieldId}'); if (inp) { inp.value = imageUrl; inp.dispatchEvent(new Event('input', { bubbles: true })); } })\" placeholder=\"URL hình ảnh hoặc chọn từ thư viện\" class=\"flex-1 px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition\">";
