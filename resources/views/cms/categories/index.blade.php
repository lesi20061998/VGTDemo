                    ...
                    <tr class="hover:bg-gray-50 transition" style="border-left: {{ $category->level * 4 }}px solid #3b82f6;">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center space-x-2">
                                <a href="{{ isset($currentProject) && $currentProject ? route('project.admin.categories.edit', [$currentProject->code, $category]) : route('cms.categories.edit', $category) }}" title="Sửa" class="p-2 text-gray-600 hover:bg-blue-100 hover:text-blue-600 rounded-lg transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <form method="POST" action="{{ isset($currentProject) && $currentProject ? route('project.admin.categories.destroy', [$currentProject->code, $category]) : route('cms.categories.destroy', $category) }}" class="inline" onsubmit="return confirm('Bạn có chắc muốn xóa?')">
                                    @csrf @method('DELETE')
