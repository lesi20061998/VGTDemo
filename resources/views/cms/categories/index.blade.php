...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ isset($currentProject) && $currentProject ? route('project.admin.categories.create', $currentProject->code) : route('cms.categories.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:shadow-lg transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center space-x-2">
                                @if(request('status') === 'trashed')
                                    <form method="POST" action="{{ isset($currentProject) && $currentProject ? route('project.admin.categories.restore', [$currentProject->code, $category->id]) : route('cms.categories.restore', $category->id) }}" class="inline">
                                        @csrf @method('PATCH')
                                        <button type="submit" title="Khôi phục" class="p-2 text-gray-600 hover:bg-green-100 hover:text-green-600 rounded-lg transition">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"></path></svg>
                                        </button>
                                    </form>
                                    <form method="POST" action="{{ isset($currentProject) && $currentProject ? route('project.admin.categories.force-delete', [$currentProject->code, $category->id]) : route('cms.categories.force-delete', $category->id) }}" class="inline" onsubmit="return confirm('Bạn có chắc muốn xóa vĩnh viễn?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" title="Xóa vĩnh viễn" class="p-2 text-gray-600 hover:bg-red-100 hover:text-red-600 rounded-lg transition">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
                                    </form>
                                @else
                                    <a href="{{ isset($currentProject) && $currentProject ? route('project.admin.categories.edit', [$currentProject->code, $category->id]) : route('cms.categories.edit', $category->id) }}" title="Sửa" class="p-2 text-gray-600 hover:bg-blue-100 hover:text-blue-600 rounded-lg transition">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <form method="POST" action="{{ isset($currentProject) && $currentProject ? route('project.admin.categories.destroy', [$currentProject->code, $category->id]) : route('cms.categories.destroy', $category->id) }}" class="inline" onsubmit="return confirm('Bạn có chắc muốn chuyển danh mục này vào thùng rác?')">
                                        @csrf @method('DELETE')
