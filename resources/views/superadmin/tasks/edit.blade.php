...
@section('title', 'Cập nhật Task')
@section('page-title', 'Cập nhật Task')

@section('content')
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form method="POST" action="{{ route('superadmin.tasks.update', $task) }}">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ $task->title }}" required class="w-full border rounded-lg px-4 py-2">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Mô tả</label>
                    <textarea name="description" rows="4" class="w-full border rounded-lg px-4 py-2">{{ $task->description }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" value="{{ $task->due_date?->format('Y-m-d') }}" class="w-full border rounded-lg px-4 py-2">
                    </div>
                </div>
            </div>
