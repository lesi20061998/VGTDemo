...
@section('title', 'Cấu hình Vai trò | Super Admin')
...
<div class="px-6 py-8 w-full max-w-7xl mx-auto">
    ...
    <form action="{{ route('superadmin.roles.update', $role->id) }}" method="POST" id="edit-form">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                    <div class="flex justify-between items-center mb-4 border-b pb-2">
                        ...
                        <span class="text-xs font-semibold text-gray-400">ID: #{{ $role->id }}</span>
                    </div>
                    
                    <div class="space-y-4">
                        <div>
                            <x-form.label value="Mã Role (Không thể sửa)" />
                            <x-form.input name="name" :value="$role->name" disabled class="bg-gray-100 text-gray-500 opacity-70 cursor-not-allowed" />
                        </div>
                        <div>
                            <x-form.label value="Tên hiển thị" required="true" />
                            <x-form.input name="display_name" :value="old('display_name', $role->display_name)" required="true" />
                        </div>
                        <div>
                            <x-form.label value="Mô tả chi tiết" />
                            <x-form.textarea name="description" :value="old('description', $role->description)" rows="3" />
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-8 pt-6 border-t border-red-100">
                    <button type="button" class="w-full py-2 px-4 bg-white border border-red-200 text-red-600 rounded-lg hover:bg-red-50 hover:border-red-300 font-medium transition-colors flex items-center justify-center" onclick="if(confirm('Bạn có chắc chắn muốn xóa vĩnh viễn Role này?')) { document.getElementById('delete-form').submit(); }">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
                    ...
                    @php
                        $rolePerms = old('permissions', $role->permissions->pluck('id')->toArray());
                    @endphp
    ...
    @if(!in_array($role->name, ['account', 'dev', 'superadmin']))
    <form id="delete-form" action="{{ route('superadmin.roles.destroy', $role->id) }}" method="POST" class="hidden">
        @csrf
        @method('DELETE')
    </form>
    @endif
