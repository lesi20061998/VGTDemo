...
<div class="max-w-4xl mx-auto">
    <form method="POST" action="{{ route('superadmin.employees.update', $employee) }}" class="bg-white rounded-lg shadow-sm p-6">
        @csrf @method('PUT')
        <div class="space-y-6">
            <div>
            ...
            <div>
                ...
                <select name="position" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600">
                    ...
                    <option value="Nhân viên" {{ old('position', $employee->position) == 'Nhân viên' ? 'selected' : '' }}>Nhân viên</option>
                    <option value="Trưởng nhóm" {{ old('position', $employee->position) == 'Trưởng nhóm' ? 'selected' : '' }}>Trưởng nhóm</option>
                    <option value="Giám đốc" {{ old('position', $employee->position) == 'Giám đốc' ? 'selected' : '' }}>Giám đốc</option>
                </select>
            ...
            <div>
                ...
                @if($isDepartmentManager)
                    ...
                    <input type="text" value="{{ ucfirst(str_replace('_', ' ', $employee->department)) }}" disabled class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100">
                @else
                <select name="department" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600">
                    ...
                    <option value="truyen_thong" {{ old('department', $employee->department) == 'truyen_thong' ? 'selected' : '' }}>Bộ Phận Truyền Thông</option>
                    <option value="ky_thuat" {{ old('department', $employee->department) == 'ky_thuat' ? 'selected' : '' }}>Bộ Phận Kỹ Thuật</option>
                    <option value="thiet_ke" {{ old('department', $employee->department) == 'thiet_ke' ? 'selected' : '' }}>Bộ Phận Thiết Kế</option>
                    <option value="hanh_chanh" {{ old('department', $employee->department) == 'hanh_chanh' ? 'selected' : '' }}>Bộ Phận Hành Chánh Nhân Sự</option>
                    <option value="ke_toan" {{ old('department', $employee->department) == 'ke_toan' ? 'selected' : '' }}>Kế Toán</option>
                    <option value="kinh_doanh" {{ old('department', $employee->department) == 'kinh_doanh' ? 'selected' : '' }}>Bộ Phận Kinh Doanh</option>
                </select>
                @endif
            ...
            <div>
                ...
                <select name="superadmin_role" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600">
                    ...
                    <option value="superadmin" {{ old('superadmin_role', $employee->superadmin_role) == 'superadmin' ? 'selected' : '' }}>SuperAdmin (Full quyền)</option>
                    <option value="director" {{ old('superadmin_role', $employee->superadmin_role) == 'director' ? 'selected' : '' }}>Director (Giám đốc vận hành)</option>
                    <option value="account" {{ old('superadmin_role', $employee->superadmin_role) == 'account' ? 'selected' : '' }}>Account (Kinh doanh)</option>
                    <option value="dev" {{ old('superadmin_role', $employee->superadmin_role) == 'dev' ? 'selected' : '' }}>Dev (Kỹ thuật)</option>
                </select>
            ...
            @else
            <input type="hidden" name="superadmin_role" value="{{ $employee->superadmin_role }}">
            <input type="hidden" name="is_department_manager" value="{{ $employee->is_department_manager ? '1' : '0' }}">
            @endif
            ...
            <div>
                ...
                <select name="department_role" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600">
                    <option value="">-- Chọn role --</option>
                    <option value="truong_phong" {{ old('department_role', $employee->department_role) == 'truong_phong' ? 'selected' : '' }}>Trưởng phòng</option>
                    <option value="pho_phong" {{ old('department_role', $employee->department_role) == 'pho_phong' ? 'selected' : '' }}>Phó phòng</option>
                    <option value="nhan_vien" {{ old('department_role', $employee->department_role) == 'nhan_vien' ? 'selected' : '' }}>Nhân viên</option>
                </select>
            ...
            <div>
                ...
                <select name="manager_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600">
                    ...
                    @foreach(\App\Models\Employee::where('is_active', true)->where('id', '!=', $employee->id)->get() as $emp)
