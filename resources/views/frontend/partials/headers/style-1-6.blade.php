{{-- Header Style 1-6: Logo giữa, menu 2 bên --}}
@php
    $showSearch = setting('show_search', true);
    $showCart = setting('show_cart', true);
    $showAccount = setting('show_account', false);
    $halfCount = $navMenu?->items ? ceil($navMenu->items->count() / 2) : 0;
    $leftItems = $navMenu?->items?->take($halfCount);
    $rightItems = $navMenu?->items?->skip($halfCount);
@endphp
...
<header class="shadow-sm relative z-50" style="background-color: {{ $headerBg }}; color: {{ $headerText }};">
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center py-4">
            {{-- Left Menu --}}
            @if($leftItems && $leftItems->count() > 0)
            ...
            {{-- Right Menu + Icons --}}
            @if($rightItems && $rightItems->count() > 0)
            <nav class="hidden lg:flex items-center gap-6 flex-1 justify-end">
            ...
            <div class="hidden lg:flex items-center gap-3 flex-1 justify-end">
                @if($showSearch)
                <button type="button" onclick="toggleSearchModal()" class="p-2 hover:bg-gray-100 rounded-full transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                </button>
                @endif
                @if($showCart)
